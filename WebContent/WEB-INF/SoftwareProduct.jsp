...
<!-- 	=======================导航栏   结束 ================================== -->
            	...
            	<div class="choose">
					<c:forEach var="authoritiy" items="${authorities}">
	                   	<c:if test="${authoritiy=='SoftwareProductOperate'}">
							  ...
							  <input type="button" value="复制添加" class="bToggle chooseBtn CopyandAdd">
	                   	</c:if>
	               	</c:forEach>
			    </div>
			...
			<table border="1" cellspacing="0" cellspadding="0" id="table1">
				<tr style="background:#bfbfbf;">
					...
					<td style="width:11.25%;">单价(RMB)</td>
					<td style="width:7.33%;min-width:95px">上次更新</td>
				...
				<c:forEach var="orderInfo" items="${datas}" varStatus="status">
					<c:if test="${status.index>0}">
						<tr class="tbody_tr">
							...
							<td class="Model overhid tdClick" title="${orderInfo['Model']}">${orderInfo['Model']}</td> 
							<td  class="PackageName overhid tdClick" title="${orderInfo['PackageName']}">${orderInfo['PackageName']}</td>   
							<td class="PackageClassify tdClick">${orderInfo['PackageClassify']}</td>  
							<td class="Brand tdClick">${orderInfo['Brand']}</td>     
							<td class="t1_UnitPrice tdClick">${orderInfo['Price']}</td>
							<td class="LastUpdate tdClick" title="${orderInfo['OperatingTime']}">${orderInfo['OperatingTime']}</td>   
							<td class="Remarks tdClick" title="${orderInfo['Remarks']}">${orderInfo['Remarks']}</td>    
			...
			<div class="info_add" style="display: none;">
				...
				<div class="ProductMsg">
					<ul class="ProductMsg_leftul">
						...
						<li>
							<span class="ProductMsg_span">时薪(RMB)</span>
			...
			<div class="info_update" style="display: none;">
				...
				<div class="edit_btn">
					...
					<input type="button" value="删除" class="bToggle" id="update_delete">
				</div>
			</div>
			
			
			<!-- 复制添加软件产品 -->
			<div class="info_update2" style="display: none;">
				<div class="title">复制添加已选的软件产品</div>
				<div class="infoAdd_close">关闭</div>
				<div class="ProductMsg">
					<ul class="ProductMsg_leftul">
						<li>
				...
				<div class="edit_btn">
					...
					<input type="button" value="取消" class="bToggle" id="update_cancle2">
				</div>
			</div>
