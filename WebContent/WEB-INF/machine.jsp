...
<!-- <link rel="stylesheet" href="src/css/modules/serviced/machine.css" type="text/css">  -->
<link rel="stylesheet" href="css/modules/serviced/machine-1a22263fb5.min.css" type="text/css"> 
<style>
	.content {
	...
	#global_add_update_module_add, #global_add_update_module_update, .152px {
		top: 10px;
	}
    ...
    <div id="bsCompatible_cssSticker_sticker">
		...
		<!-- 	=======================导航栏   结束 ================================== -->
					...
					<form id="top_text_from" name="top_text_from" method="post" action="GetMachineDetailsRoute">
						...
						<div class="isNormalPage" style="float:left">
							<input type="button" value="验收异常统计" class="btn btn-info" style="margin: 30px 0 10px 25px;">
						</div>
				 </form>
				<!-- 主页面表格 -->
						...
						<tbody>
							<c:forEach var="orderInfo" items="${machine}" varStatus="status">
								<c:if test="${status.index>0}">
									<tr>
										<td value="${orderInfo['ID']}" data-targetmodule="update" class="editID">${status.index+(currentPage-1)*10}</td>
										...
										<td class="DeatailDate"  style="display:none">${orderInfo['DeatailDate']}</td>
										<%-- <td class="t0"  style="display:none">${orderInfo['InstalledTime']}</td>
										<td class="t1"  style="display:none">${orderInfo['InstallTime']}</td> --%>
										<td title="${orderInfo['InstalledTime']}" class="td_InstalledTime">${orderInfo['InstalledTime']}</td>
										    
										<%-- <c:choose>
										    <c:when test="${orderInfo['InstallTime'] != ''}">
										        <td title="${orderInfo['InstallTime']}" class="td_InstalledTime">${orderInfo['InstallTime']}</td>
										    </c:when>
										    <c:otherwise>
										     	<td title="${orderInfo['InstalledTime']}" class="td_InstalledTime">${orderInfo['InstalledTime']}</td>
										    </c:otherwise>
										</c:choose> --%>
										<td class="InstalledTime_all" title="点击查看详细装机时间">${orderInfo['TotalDays']}</td>
										<td data-ivalue="${orderInfo['Status']}" class="td_Status"></td>
										<td class="isNormal">${orderInfo['IsNormal']}</td>
				...
				<div class="global_add_update_module" id="global_add_update_module_add" data-classify="add" data-parents="addANDupdate">
					...
					<div class="global_add_update_module_body">
						<div class="global_add_update_module_bodyin">
				    		...
				    		<fieldset class="machineTime"><legend>装机时间&nbsp;</legend>
				    			<div class="add_times">
						    		<span>第 1 次&nbsp;&nbsp;</span>
						    		<span>起始时间&nbsp;</span>
						    		<input type="date" id="add_startTime" class="">
						    		<span class="add_end">结束时间&nbsp;</span>
									<input type="date" id="add_endTime" class="">
									<span class="add_allTime">共<i>X</i>天</span>
								</div>
				    		</fieldset>
					...
					<div class="global_add_update_module_foot">
						<div class="global_add_update_module_footin">
							<input type="button" value="提交" class="btn btn-success global_add_update_module_submit" id="add_submit">
				...
				<div class="global_add_update_module" id="global_add_update_module_update" data-classify="update" data-parents="addANDupdate">
					...
					<div class="global_add_update_module_body">
						<div class="global_add_update_module_bodyin">
									...
									<div class="row">
										...
										<div class="col-md-6 col-lg-6">
											<div class="form-horizontal">
											    ...
											    <div class="form-group">
											        <label for="" class="col-sm-3 control-label isNormal_title" style="padding-left:0">是否正常验收</label>
											        <div class="col-sm-9 isNormal">
											        	<select class="form-control">
															  <option text='未选择'>未选择</option>
															  <option text='是'>是</option>
															  <option text='否'>否</option>
															  <option text='不涉及'>不涉及</option>
														</select>
											        </div>
											    </div>
				    		...
				    		<fieldset class="edit_machineTime"><legend>装机时间&nbsp;<i class="glyphicon glyphicon-plus-sign addmachineTime" aria-hidden="true"></i></legend>
					...
					<div class="global_add_update_module_foot">
						<div class="global_add_update_module_footin">
							<input type="button" value="提交" class="btn btn-success global_add_update_module_submit" id="edit_submit">
							<input type="button" value="取消" class="btn btn-warning global_add_update_module_cancel">
						</div>
					</div>
				</div>
				...
				<!-- 装机时间模块开始 -->
				<div class="mask"></div>
				<div class="InstalledTime_Box">
					<div class="InstalledBox_title">
						<span>装机时间详情</span>
						<i></i>
					</div>	
					<div class="InstalledBox_con">
					<!-- 	<div class="show_times">
						    		<span>第 1 次&nbsp;&nbsp;</span>
						    		<span>起始时间&nbsp;</span>
						    		<input type="date" id="show_startTime" disabled="disabled">
						    		<span class="show_end">结束时间&nbsp;</span>
									<input type="date" id="show_endTime"  disabled="disabled">
									<span class="show_allTime">共<i>X</i>天</span>
						</div> -->
					</div>
				</div>
				<!-- 装机时间模块结束 -->
