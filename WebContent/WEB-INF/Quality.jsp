	...
	<div class="contain">
		...
		<!-- 	=======================导航栏   开始 ================================== -->
			...
			<div class="breadNav">
				...
				<span class="breadR">商务部 &nbsp; &gt;  &nbsp;文档制作 &nbsp; &gt;  &nbsp;</span>
				<span style="color:#18b4f0">质量证明</span>
			</div>
	 	 ...
	 	 <c:choose>
				<c:when test="${queryType == 'common'}">
					<c:set var="queryUrl"
					value="Quality?type1=${classify1 }&searchContent1=${parameter1}&selected=&currentPage=">
					</c:set>
				</c:when>
				<c:when test="${queryType == 'singleSelect'}">
					<c:set var="queryUrl"
					value="GetQualityRoute?type1=${classify1 }&searchContent1=${parameter1}&selected=singleSelect&currentPage=">
					</c:set>
				</c:when>
				<c:otherwise>
					<c:set var="queryUrl"
					value="GetQualityRoute?type1=${classify1 }&searchContent1=${parameter1}&type2=${classify2}&searchContent2=${parameter2}&selected=mixSelect&currentPage=">
					</c:set>
				</c:otherwise>
...
$(document).on("click",".contract-edit",function(){	
	...
	console.log(ID)
   $('.MailBar_cover_color').slideDown(300);
   $('.contract_update').slideDown(300);
