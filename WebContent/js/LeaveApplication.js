...
$(".preview").click(function(){
	...
	$(".contract_update2 .update_id").text(tr.find(".preview").attr('value'));
	 ...
	 $('.cover-color').slideDown();
	    $('.contract_update2').slideDown();
})
...
$(".Review1").click(function(){
		...
		//判断是否有权限2
		/*var Toman = $(this).parent().find('.ToList').text();
		var index = Toman.indexOf('<');
		Toman = Toman.slice(index+1,Toman.length-1);
		var username = $('.eou-container-r .m-admin .u-span').text();
		if(Toman == username){
			$(".contract_update .ReasonContent").val('');
			$(".contract_update .Password2").val('');
			$('.cover-color').show();
