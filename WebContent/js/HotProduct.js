...
//导出功能
$('.classify_export').click(function(){
	var ProductCategory = '';
	var Model = '';
	var type = $('.input-group-btn button:nth-child(1)').text();
	if(type == '选择字段'){
	}
	if(type == '产品类型'){
		ProductCategory = $('#classify_search option:selected').text();
	}
	if(type == '产品型号'){
	...
	var Column = '';
	if($('.col_Year_2016').attr('rank') == 'rank'){
		 Column = 'Year_2016';
	}
	if($('.col_Year_2017').attr('rank') == 'rank'){
		 Column = 'Year_2017';
	}
	if($('.col_Year_2018').attr('rank') == 'rank'){
		 Column = 'Year_2018';
	}
	$.ajax({
		type:'post',
		...
		success:function(data){
			window.location.href='/cfChicken8/'+data;
		}
	})
})
