...
$(document).on('click','#update_delete',function(){
		$.MsgBox_Unload.Confirm("提示", "确定要删除该条产品记录?");
...
$('#table1 td.tdClick').click(function(event){
	$(this).parent().addClass('ChooseTr').siblings().removeClass('ChooseTr');
	var tr = $(this).parent();
	tr.siblings('.tbody_tr').css({
		'background-color':'#fff'
	});
	tr.css({
...
$('.CopyandAdd').click(function(){
	var tr = $('.ChooseTr');
	 if (event && event.stopPropagation) {
	    ...
	    var info_update2 = $('.info_update2');
	    info_update2.find('input[name="Model"]').val(tr.find('.Model').html());
	    info_update2.find('input[name="PackageName"]').val(tr.find('.PackageName').html());
	    info_update2.find('input[name="PackageClassify"]').val(tr.find('.PackageClassify').html());
	    info_update2.find('input[name="Brand"]').val(tr.find('.Brand').html());
	    info_update2.find('.Remarks').val(tr.find('.Remarks').html());
	    info_update2.find('input[name="HourlyWage"]').val(tr.find('.HourlyWage').html());
	    info_update2.find('input[name="Cycle"]').val(tr.find('.Cycle').html());
	    info_update2.find('input[name="TransportAllowance"]').val(tr.find('.TransportAllowance').html());
	    info_update2.find('input[name="AccommodationAllowance"]').val(tr.find('.AccommodationAllowance').html());
	    info_update2.find('input[name="MissionAllowance"]').val(tr.find('.MissionAllowance').html());
	    $('.cover-color').fadeIn();
	    info_update2.show();
});
$('#update_submit2').click(function () {
    var PackageName = $('.info_update2 input[name="PackageName"]').val();
    var Model = $('.info_update2 input[name="Model"]').val();
    var HourlyWage = $('.info_update2 input[name="HourlyWage"]').val();
    var PackageClassify = $('.info_update2 input[name="PackageClassify"]').val();
    var Cycle = $('.info_update2 input[name="Cycle"]').val();
    var Brand = $('.info_update2 input[name="Brand"]').val();
    var TransportAllowance = $('.info_update2 input[name="TransportAllowance"]').val();
    var AccommodationAllowance = $('.info_update2 input[name="AccommodationAllowance"]').val();
    var MissionAllowance = $('.info_update2 input[name="MissionAllowance"]').val();
    var Remarks = $('.info_update2 .Remarks').val();
    $.ajax({
        type: 'post',
        url: 'SoftwareProductOperate',
        data: {
