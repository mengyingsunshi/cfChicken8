...
public class SoftwareProductDao {
	...
	public boolean update(SoftwareProduct product){
		SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		String sql = "update t_software_product set PackageName=?,Model=?,HourlyWage=?,PackageClassify=?,Cycle=?,Brand=?,"
				+ "TransportAllowance=?,MissionAllowance=?,AccommodationAllowance=?,Remarks=?,OperatingTime=? where ID=?";
		Object[] param =  new Object[12];
		param[0] = product.getPackageName();
		param[1] = product.getModel();
		param[2] = product.getHourlyWage();
		param[3] = product.getPackageClassify();
		param[4] = product.getCycle();
		param[5] = product.getBrand();
		param[6] = product.getTransportAllowance();
		param[7] = product.getMissionAllowance();
		param[8] = product.getAccommodation();
		param[9] = product.getRemarks();
	...
	public boolean delete(int id){
		...
		String sql = "delete from t_software_product where ID = ?";
