...
//		System.out.println(area);
	...
	public Map<String, Integer> getRequirementNum(Connection conn){
		Map<String, Integer> total = new HashMap<>();
		String sql = "select COUNT(ID) Count,DATE_FORMAT(RequirementDate,'%Y') Years from "
				+ "t_requirement where RequirementDate >= '2016-01-01' GROUP BY "
				+ "DATE_FORMAT(RequirementDate,'%Y') ORDER BY Years ";
		...
		try {
			...
			while (resultSet.next()) {
				total.put(resultSet.getString("Years"), resultSet.getInt("Count"));	
			}
	...
	public Map<String, Integer> getDealNum(Connection conn){
		Map<String, Integer> deal = new HashMap<>();
		String sql = "select COUNT(ID) Count,DATE_FORMAT(RequirementDate,'%Y') Years from "
				+ "t_requirement where RequirementDate >= '2016-01-01' AND WhetherSingle = '1' "
				+ "GROUP BY DATE_FORMAT(RequirementDate,'%Y') ORDER BY Years ";
		...
		try {
			...
			ResultSet resultSet = pStatement.executeQuery();
			while (resultSet.next()) {
				deal.put(resultSet.getString("Years"), resultSet.getInt("Count"));	
			}
