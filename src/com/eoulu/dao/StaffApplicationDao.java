...
public class StaffApplicationDao {
	...
	public List<Map<String, Object>> getAllData(Page page,String user,String authority,String startTime,String endTime) {
		...
		if(authority.equals("no")){
			...
			sql = "select * from t_staff_application where (RealName =? or ToList like ?)";
				
			param = new Object[] { user,"%"+user+"%",(page.getCurrentPage() - 1)*10, page.getRows() };
			if(!startTime.equals("")){
				sql=sql.concat(" AND DATE_FORMAT(StartTime,'%Y-%m') between ? and ?");
				param = new Object[] {user,"%"+user+"%",startTime,endTime,(page.getCurrentPage() - 1)*10, page.getRows()};
			}
		}
	...
	public int getCounts(String user,String authority,String startTime,String endTime) {
		...
		if(authority.equals("no")){
			...
			sql = "select count(ID) from t_staff_application where (RealName =? or ToList like ?) ";
			param = new Object[] { user,"%"+user+"%"};
			if(!startTime.equals("")){
				sql=sql.concat(" AND DATE_FORMAT(StartTime,'%Y-%m') between ? and ?");
				param = new Object[] { user,"%"+user+"%",startTime,endTime};
			}
		}
		List<Map<String, Object>> ls = db.QueryToList(sql, param);
		int count = 0;
