...
public class OrderDao {
	...
	public List<Map<String, Object>> getShippedCountPerMonth(int type,String startTime,String endTime){
		DBUtil dbUtil = new DBUtil();
		String sql = "SELECT count(ID) Count,DATE_FORMAT(t_order.ActualDelivery,'%Y-%m') months from t_order "
				+ "where ActualDelivery >= ? AND ActualDelivery <= ? AND PageType = ? GROUP BY months";
	...
	public List<Map<String, Object>> getShippedCountPerYear(int type,String startTime){
		DBUtil dbUtil = new DBUtil();
		String sql = "SELECT count(ID) Count,DATE_FORMAT(t_order.ActualDelivery,'%Y') years from t_order "
				+ "where ActualDelivery >= ?  AND PageType = ? GROUP BY years";
		
		return dbUtil.QueryToList(sql, new Object[]{startTime,type});
