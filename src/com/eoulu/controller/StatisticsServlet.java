...
public class StatisticsServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if(loadType.equals("yearSales")){
			String date = request.getParameter("date");
			StatisticsService statisticsService = new StatisticsServiceImpl();
			
			response.getWriter().write(new Gson().toJson(statisticsService.getSalesStage(date)));
		}else{
			...
			request.setAttribute("volumn",new Gson().toJson(statisticsService.getSingleQuantity(start, end)));
			request.setAttribute("targetValue", statisticsService.getTargetValue(start2,end2));
